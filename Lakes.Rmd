...
Now restrict the data to only the study lakes and those data collected in 2017:  
Study lakes:            
LAKE_ID     WATER  
1501ROC0985 Rockland Lake  
1101ROU1089 Round Lake  
1310NAS0034 Nassau Lake  
1301LAW0207 Lawson Lake  
0301LON0154 Long Pond  
0602DEA0036 Dean Pond  
1102BAB1109 Babcock Lake  
1302WAC0117 Lake Waccabuc  
0906BUT0054 Butterfield Lake  
0602SON0072 Song Lake  
```{r, echo=FALSE}
#restrict to 2017
data<-data[data$SAMPLE_DATE>'2017-01-01',]
data<-data[data$SAMPLE_DATE<'2018-01-01',]

#restrict to study lakes
stations = c('1501ROC0985','1101ROU1089','1310NAS0034','1301LAW0207','0301LON0154','0602DEA0036','1102BAB1109','1302WAC0117','0906BUT0054','0602SON0072')
nstations = length(stations)

for(i in 1:nstations){
  temp <- data[data$LAKE_ID == stations[i],]
  if(exists("dataset")){
...
dataset<-dataset[!is.na(dataset$LAKE_ID),]
data<-dataset

rm(list = c('i','nstations','stations','dataset'))


#remove rows with NA values in crucial fields
#then delete na values
#data<-data[!is.na(data$LAKE_ID),]
#data<-data[!is.na(data$LOCATION_ID),]
#data<-data[!is.na(data$SAMPLE_DATE),]
